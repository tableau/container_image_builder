...
function mariadb() {
  curl --location --output mariadb.tar.gz https://dlm.mariadb.com/997528/Connectors/odbc/connector-odbc-3.1.7/mariadb-connector-odbc-3.1.7-ga-rhel8-x86_64.tar.gz
}
...
function microsoft_sql_server() {
  curl https://packages.microsoft.com/config/rhel/8/prod.repo >microsoft-rhel8.repo
}

function mysql() {
  curl --location --output mysql.rpm https://dev.mysql.com/get/mysql80-community-release-el8-4.noarch.rpm
}
